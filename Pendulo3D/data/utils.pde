...
void saveConfig(String filename) {
  ...
  // Creamos un objeto JSON por cada soundSet
  for (int i = 0; i < soundSets.length; i++) {
    JSONObject setObj = new JSONObject();
    
    setObj.setString("folder", soundSets[i]);
    
    JSONArray filesArray = new JSONArray();
    for (String f : soundFiles[i]) filesArray.append(f);
    setObj.setJSONArray("files", filesArray);
    
    JSONArray volumesArray = new JSONArray();
    for (float v : soundVolumes[i]) volumesArray.append(v);
    setObj.setJSONArray("volumes", volumesArray);
    
    setsArray.append(setObj);
  }
  
  config.setJSONArray("soundSets", setsArray);

  config.setInt("currentSoundSet", currentSoundSet);
...
void loadConfig(String filename) {
  ...
  currentSoundSet   = config.getInt("currentSoundSet", 0);
  ...
  for (int i = 0; i < numSets; i++) {
    ...
    // Carpeta
    soundSets[i] = setObj.getString("folder");
